...
cdef class PressureThresholdModel(DiffusionModel):
    ...
    cdef int _advance_model(
        ...
        # cdef unsigned int child_range_end
        ...
        for _ in range(q):
            ...
            for edge_idx in range(self.starts[node], range_end):
                ...
                if seen_set.find(child) == seen_set.end():
                    ...
                    # Update outgoing influence **only at the moment of activation** for the activated child
                    # child_range_end = len(self.edges)
                    # if child + 1 < len(self.starts):
                    #     child_range_end = self.starts[child + 1]
                    # for edge_idx in range(self.starts[child], child_range_end):
                    #     # # Don't update edge weights on already activated neighbors
                    #     # if seen_set.find(self.edges[edge_idx]) != seen_set.end():
                    #     #     continue
                    #     updated_weight = self.influence[edge_idx] + self.alpha * buckets[child]
